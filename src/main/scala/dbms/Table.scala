...
class Table(file: RandomAccessFile) {
  private val numPages = 10
  ...
  def toChar(s: String): Char = if (s.length == 1) s.head else '\u0000'
  ...
  def removeDash(s: String): String = {
    val ss = s.substring(0,3) + s.substring(4,6) + s.substring(7,11)
    ss
  }
  def parseCSVLine(line: String, rId: Int): Record = {
